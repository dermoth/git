...
static int ispipe(const char *name)
{
	...
	/* Unnamed pipes are usually implemented through a symlink */
	if (!lstat(name, &st)) {
		...
		if (!stat(name, &st)) {
			...
			/* We have a symlink that points to a pipe. If its target once
			 * resolved (relative paths & all) doesn't really exist we
			 * can safely assume it's a special file and use it */
			struct strbuf sb = STRBUF_INIT;
			strbuf_realpath(&sb, name, 0);
			/* We're abusing strbuf_realpath here, it may append
			 * pipe:[NNNNNNNNN] to an abs path */
			if (stat(sb.buf, &st))
				return 1; /* link target doesn't exists but is pipe */
		}
	}
	return 0;
}
...
static int populate_from_pipe(struct diff_filespec *s, const char *name)
{
	...
	f = xopen(name, O_RDONLY);
	ret = populate_from_fd(s, f);
	close(f);
	return ret;
}
